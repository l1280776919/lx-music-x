  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-2 text-zinc-300 truncate max-w-[240px]">
        <Music2 class="w-3.5 h-3.5 text-brand-400 flex-shrink-0" />
        <span class="font-medium truncate">{{ songInfo || '洛雪音乐' }}</span>
      </div>

      <!-- 控制按键: 切歌、播放/暂停、字号、锁定 -->
      <div class="flex items-center gap-3">
        <button class="hover:text-brand-400 cursor-pointer transition flex items-center justify-center p-1" title="上一首" @click="controlAction('prev')">
          <SkipBack class="w-3.5 h-3.5 fill-current" />
        </button>
        <button class="hover:text-brand-400 cursor-pointer transition flex items-center justify-center p-1 font-bold" :title="isPlaying ? '暂停' : '播放'" @click="controlAction('toggle-play')">
          <Pause v-if="isPlaying" class="w-3.5 h-3.5 fill-current" />
          <Play v-else class="w-3.5 h-3.5 fill-current ml-0.5" />
        </button>
        <button class="hover:text-brand-400 cursor-pointer transition flex items-center justify-center p-1" title="下一首" @click="controlAction('next')">
          <SkipForward class="w-3.5 h-3.5 fill-current" />
        </button>

        <span class="w-[1px] h-3 bg-white/20"></span>

        <button class="hover:text-brand-400 cursor-pointer text-xs px-1" title="调小字号" @click="changeFontSize(-2)">
          A-
        </button>
        <button class="hover:text-brand-400 cursor-pointer text-xs px-1" title="调大字号" @click="changeFontSize(2)">
          A+
        </button>
        ...
        <button
          class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/10 hover:bg-brand-500 hover:text-white transition cursor-pointer font-medium text-[11px]"
    ...
    <div
      v-if="showLockToast"
      class="absolute top-2 px-4 py-1.5 rounded-full bg-brand-500/90 text-white text-xs shadow-lg backdrop-blur z-50 animate-fade-in"
    >
      {{ lockToastMessage }}
    </div>
    ...
    <div class="text-center space-y-1.5 pointer-events-none transition-all duration-200">
      <div
        ...
        :style="{ fontSize: `${fontSize}px`, textShadow: '0 0 12px rgba(45, 168, 108, 0.45)' }"
      >
