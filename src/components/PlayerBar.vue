<template>
  <footer class="h-20 w-full px-6 flex items-center justify-between bg-white/80 dark:bg-zinc-950/80 border-t border-zinc-200/60 dark:border-zinc-800/60 backdrop-blur-2xl select-none z-30 transition-colors flex-shrink-0">
    <!-- 左侧: 歌曲封面与信息 (点击展开全屏沉浸流光界面) -->
    <div class="flex items-center gap-3.5 w-1/4 min-w-[240px]">
      <div
        class="w-13 h-13 rounded-2xl relative overflow-hidden group cursor-pointer shadow-md bg-gradient-to-br from-zinc-800 to-zinc-950 flex-shrink-0 flex items-center justify-center"
      ...
      <div class="overflow-hidden flex-1 cursor-pointer" @click="playerStore.isDetailOpen = true">
        <div class="flex items-center gap-2">
          ...
          <!-- 动态跳动音阶微标 -->
          <div v-if="playerStore.isPlaying" class="flex items-end gap-[2px] h-3.5 flex-shrink-0">
    ...
    <div class="flex flex-col items-center gap-1.5 flex-1 max-w-xl px-4">
      <div class="flex items-center gap-5">
        ...
        >
          <Shuffle v-if="playerStore.playMode === 'random'" class="w-4 h-4 text-emerald-500" />
          <Repeat1 v-else-if="playerStore.playMode === 'single'" class="w-4 h-4 text-emerald-500" />
          <Repeat v-else class="w-4 h-4" />
        </button>

        <!-- 上一首 -->
        <button
          class="w-9 h-9 rounded-full flex items-center justify-center text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-900 transition active:scale-90"
          title="上一首 (Alt + Left)"
        ...
        <!-- 播放 / 暂停 主按键 (大圆形现代按键) -->
        <button
          class="w-11 h-11 rounded-full bg-emerald-500 hover:bg-emerald-400 text-white flex items-center justify-center shadow-lg shadow-emerald-500/30 transition-transform active:scale-95"
      ...
      <div class="w-full flex items-center gap-3 text-[11px] text-zinc-400 font-mono">
        <span class="w-10 text-right tabular-nums">{{ formatTime(playerStore.currentTime) }}</span>
        <div
        ...
        >
          ...
          <!-- 悬浮滑块圆点 -->
          <div
            class="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-white shadow-md border-2 border-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"
            :style="{ left: `${playerStore.progressPercent}%` }"
          ></div>
        </div>
        <span class="w-10 tabular-nums">{{ formatTime(playerStore.duration) }}</span>
      </div>
    </div>

    <!-- 右侧: 辅助功能 (10段EQ、桌面悬浮歌词开关、音量调节) -->
    <div class="flex items-center justify-end gap-3 w-1/4 min-w-[240px]">
      <!-- 专业 10 段 EQ 音效按键 (极简现代胶囊) -->
      <button
